...
function GeoDataManagerDashboard() {
  ...
  const handleKmlUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (parsed.length === 0) {
          alert('No valid features found in this KML file. Ensure it contains Points, LineStrings, or Polygons.');
        } else {
        ...
        alert('Error parsing KML file. Please ensure it is a valid XML KML.');
      } finally {
  ...
  const handleImportKmlToDatabase = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        alert(`Successfully imported ${data.count} assets into the database!`);
        setImportedKmlList([]);
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-slate-50 font-sans text-slate-800 overflow-hidden" id="app_root">
      ...
      {/* 0. APP TOP HEADER */}
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 z-20 shadow-sm flex-shrink-0" id="app_header">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold text-slate-800 tracking-tight flex items-center gap-1.5">
              GeoData Manager <span className="text-slate-400 font-normal text-[11px] bg-slate-100 px-1.5 py-0.5 rounded-md">v2.4</span>
            </h1>
          </div>
        </div>

        {/* Center Navigation Tabs (Aesthetic & Interactive) */}
        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200/45">
          <button
        ...
        <div className="flex items-center gap-4">
          ...
          {authLoading ? (
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-indigo-50 border border-indigo-200 flex items-center justify-center text-indigo-700 text-xs font-bold font-mono shadow-inner">
                {user.email?.substring(0, 2).toUpperCase() || 'U'}
              </div>
              <span className="text-xs font-semibold text-slate-700 max-w-[120px] truncate hidden lg:inline">
                {user.email}
              </span>
              <button 
            ...
            <button
              ...
              className="py-1.5 px-3 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-lg text-xs font-bold transition-all border border-indigo-100 flex items-center gap-1.5 shadow-sm"
            >
          ...
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-750 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-sm shadow-indigo-600/10 transition-all flex items-center gap-1.5"
          >
